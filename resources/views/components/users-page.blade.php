<x-layout>
    ...
    @component('components.users', ['title' => $title, 'users' => $users])
        @if (empty($users) || $users->isEmpty())
            <tr>
                <td class="text-center text-muted" colspan="10">Data User tidak tersedia</td>
            </tr>
        @else
            @foreach ($users as $user)
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="px-6 flex py-4 justify-center">
                        <a href="{{ route('edit-users', $user->id_user) }}" button type="button"
                        ...
                        <form action="{{ route('delete-users', $user->id_user) }}" method="POST" style="display:inline;"
