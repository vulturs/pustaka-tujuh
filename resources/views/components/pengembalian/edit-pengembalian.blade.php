...
<div class="p-5 me-4 bg-slate-200 rounded-2xl mb-4">
    <div class="relative mt-8 w-1/2 flex flex-col rounded-lg bg-white bg-clip-border text-gray-700 shadow-lg">
        ...
        <div class="flex flex-col gap-4 p-6">
            ...
            <form class="form" method="post" action="{{ route('update-pengembalian', $kembali->id_pengembalian) }}">
                @method('put')
                @csrf
                    ...
                    <div class="flex p-3 pb-0">
                        <div class="flex flex-col mb-2">
                            ...
                            <input required class="font-medium text-md" placeholder="" type="text" id="id_peminjaman"
                                name="id_peminjaman" value="{{ $kembali->id_peminjaman }}" disabled>
                            <hr>
                        </div>
                        <div class="flex flex-col mb-2">
                            ...
                            <input required class="font-medium text-md" placeholder="" type="text" id="nama_angggota"
                                name="nama_angggota" value="{{ $kembali->nama_anggota }}" disabled>
                            {{-- <hr> --}}
                        </div>
                    </div>
                    <div class="bg-white border-2 border-t-purple-500 border-dashed rounded-2xl p-4 pb-5 px-5">
                        <div class="flex flex-col">
                            ...
                            <input required class="font-medium text-md" placeholder="" type="text" id="judul_buku"
                                name="judul_buku" value="{{ $kembali->judul_buku }}" disabled>
                            {{-- <hr> --}}
                        </div>
                        <div class="flex">
                            <div class="mt-1 flex flex-col">
                                ...
                                <input required class="font-medium text-sm" placeholder="" type="text" id="pengarang"
                                    name="pengarang" value="{{ $kembali->pengarang }}" disabled>
                                {{-- <hr> --}}
                            </div>
                            <div class="mt-1 flex flex-col">
                                ...
                                <input required class="font-medium text-sm" placeholder="" type="text" id="penerbit"
                                    name="penerbit" value="{{ $kembali->nama_penerbit }}" disabled>
                                {{-- <hr> --}}
                            </div>
                            <div class="mt-1 flex flex-col">
                                <span class="text-slate-400 text-sm">Kode DDC</span>
                                <input required class="font-medium text-sm" placeholder="" type="text"
                                    id="klasifikasi" name="klasifikasi" value="{{ $kembali->kode_ddc }}" disabled>
                                {{-- <hr> --}}
                            </div>
                        </div>
                ...
                <div class="flex mb-3 w-full">
                    <div class="">
                        ...
                        <input required class="font-medium text-lg" placeholder="" type="date-local"
                            ...
                            value="{{ \Carbon\Carbon::parse($kembali->tanggal_peminjaman)->format('d M Y') }}"
                    ...
                    <div class="w-full">
                        ...
                        <input required class="font-medium text-lg w-full" placeholder="" type="date-local"
                            value="{{ \Carbon\Carbon::parse($kembali->tanggal_pengembalian)->format('d M Y') }}"
                            id="tanggal_pengembalian" name="tanggal_pengembalian" readonly>
                    </div>
                    <div class="w-full">
                        <span class="text-slate-500 text-sm">Di Kembalikan Pada <i class="text-xs">(mm/dd/yyy)</i></span><br>
                        @if ($kembali->tanggal_dikembalikan <= $kembali->tanggal_pengembalian)
                            <input required class="font-medium text-lg text-green-600 w-full" placeholder=""
                                type="date" value="{{ $kembali->tanggal_dikembalikan }}"
                                id="dikembalikan" name="tanggal_dikembalikan">
                        @else
                ...
                <div class="flex w-full gap-4">
                    <div class="w-full">
                        ...
                        <select class="input01" id="id_pelanggaran" name="id_pelanggaran" autocomplete="id_pelanggaran">
                            <option value="">-- Pelanggaran --</option>
                            @forelse ($pelanggaran as $langgar)
                            @if (old('id_pelanggaran', $kembali->id_pelanggaran) == $langgar->id_pelanggaran)
                                <option value="{{ $langgar->id_pelanggaran }}" data-jenis="{{ $langgar->jenis_pelanggaran }}" selected>{{ $langgar->jenis_pelanggaran }}</option>
                            @else
                                <option value="{{ $langgar->id_pelanggaran }}" data-jenis="{{ $langgar->jenis_pelanggaran }}">{{ $langgar->jenis_pelanggaran }}</option>
                            @endif
                        @empty
                            <option value="" {{ old('id_pelanggaran') == 'null' ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="number" class="input01" id="denda" placeholder="Denda" name="denda" value="{{$kembali->denda}}" disabled>
                    </div>
                </div>
                <label>
                    <textarea name="keterangan" rows="2" placeholder="" class="input01">{{ $kembali->keterangan }}</textarea>
                    <span>Keterangan</span>
                </label>

                <input type="hidden" name="id_peminjaman" value="{{ $kembali->id_peminjaman }}">
                <input type="hidden" name="kode_buku_induk" value="{{ $kembali->kode_buku_induk }}">
...
<script>
    document.getElementById('id_pelanggaran').addEventListener('change', function() {
        ...
        } else if(jenisPelanggaran === 'Rusak'){
            document.getElementById('denda').value = 50000;
        }else {
