...
<div class="p-5 me-4 bg-slate-200 rounded-2xl mb-4">
    <div class="relative mt-10 w-1/2 flex flex-col rounded-lg bg-white bg-clip-border text-gray-700 shadow-lg">
        ...
        <div class="flex flex-col gap-4 p-6">
            ...
            <form class="form" method="post" action="{{ route('update-kunjungan', $kunjungan->id_kunjungan) }}">
                @method('put')
                @csrf

                <label>
                    <span class="text-sm text-slate-500">Nama Anggota</span>
                    <input required class="font-medium text-lg w-full" placeholder="Nama Anggota" type="text"
                        id="nama_anggota" name="nama_anggota"
                        value="{{ old('nama_anggota', $kunjungan->nama_anggota) }}" disabled>
                </label>

                <label>
                    <span class="text-sm text-slate-500">Kelas</span>
                    <input required class="font-medium text-lg w-full" placeholder="Kelas" type="text" id="kelas"
                        name="kelas" value="{{ old('kelas', $kunjungan->kelas) }}" disabled>
                </label>

                <label>
                    <textarea required="" name="tujuan_kunjungan" rows="2" placeholder="" class="input01">{{ $kunjungan->tujuan_kunjungan }}</textarea>
                    <span>Tujuan Kunjungan</span>
                </label>

                <input type="hidden" name="modified_by" value="{{ auth()->user()->id_user }}">
                <input type="hidden" id="id_anggota" value="{{ $kunjungan->id_anggota }}" name="id_anggota">
