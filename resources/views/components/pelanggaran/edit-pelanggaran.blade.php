...
<div class="p-5 me-4 bg-slate-200 rounded-2xl mb-4">
    <div class="relative mt-10 w-1/2 flex flex-col rounded-lg bg-white bg-clip-border text-gray-700 shadow-lg">
        ...
        <div class="flex flex-col gap-4 p-6">
            <form class="form" method="post" action="/administrasi/pelanggaran/{{ $pelanggaran->id_pelanggaran }}/update">
                @method('put')
                @csrf
                <label>
                    <input required placeholder="" type="text" class="input" name="jenis_pelanggaran"
                        value="{{ old('jenis_pelanggaran', $pelanggaran->jenis_pelanggaran) }}">
                    <span>Nama Lengkap</span>
                </label>

                <label>
                    <textarea required="" name="keterangan" rows="2" placeholder="" class="input01">{{ old('keterangan', $pelanggaran->keterangan) }}</textarea>
