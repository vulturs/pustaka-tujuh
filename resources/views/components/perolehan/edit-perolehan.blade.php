...
<div class="p-5 me-4 bg-slate-200 rounded-2xl mb-4">
    <div class="relative mt-10 w-1/2 flex flex-col rounded-lg bg-white bg-clip-border text-gray-700 shadow-lg">
        ...
        <div class="flex flex-col gap-4 p-6">
            <form class="form" method="post" action="/administrasi/perolehan/{{ $perolehan->id_perolehan }}/update">
                @method('put')
                @csrf
                <label>
                    <input required placeholder="" type="text" class="input" name="nama_sumber"
                        value="{{ old('nama_sumber', $perolehan->nama_sumber) }}">
                    <span>Nama Lengkap</span>
                </label>

                <label>
                    <input required placeholder="" type="text" class="input" name="no_telp"
                        value="{{ old('no_telp', $perolehan->no_telp) }}">
                    <span>Nomer Telp</span>
                </label>

                <label>
                    <input required placeholder="" type="text" class="input" name="provinsi"
                        value="{{ old('provinsi', $perolehan->provinsi) }}">
                    <span>Provinsi</span>
                </label>

                <label>
                    <input required placeholder="" type="text" class="input" name="kota_kab"
                        value="{{ old('kota_kab', $perolehan->kota_kab) }}">
